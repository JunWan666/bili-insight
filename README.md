...
- **任务与产物管理**：支持进度、取消、重试、详情、批量下载、批量删除、保留与清理，长任务状态可持续追踪。
...
- 任务中心：按视频聚合相关任务，查看阶段、进度、速度、失败原因，支持复用、取消和重试，并可跳转官方源视频。
- 产物中心：查看详细元数据，管理视频、音频、字幕、转写、关键帧和分析报告，支持批量下载、批量彻底删除及官方源链接。
...
下面的命令会从 GitHub `v1.1.0` Release 对应的源码标签下载 Compose 文件和 GHCR 配置，不需要克隆整个仓库。当前仓库为私有仓库，先准备一个同时拥有仓库读取权限和 `read:packages` 权限的 GitHub Token：

Linux/macOS：

```bash
export GITHUB_TOKEN=ghp_your_token
curl -H "Authorization: Bearer ${GITHUB_TOKEN}" -fsSL https://raw.githubusercontent.com/JunWan666/bili-insight/v1.1.0/docker-compose.yml -o docker-compose.yml
curl -H "Authorization: Bearer ${GITHUB_TOKEN}" -fsSL https://raw.githubusercontent.com/JunWan666/bili-insight/v1.1.0/ghcr-compose.env -o .env
...
Invoke-WebRequest -UseBasicParsing -Headers $headers https://raw.githubusercontent.com/JunWan666/bili-insight/v1.1.0/docker-compose.yml -OutFile docker-compose.yml
Invoke-WebRequest -UseBasicParsing -Headers $headers https://raw.githubusercontent.com/JunWan666/bili-insight/v1.1.0/ghcr-compose.env -OutFile .env
docker login ghcr.io
docker compose pull
docker compose up --detach --no-build --wait
```
