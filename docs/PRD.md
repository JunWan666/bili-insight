...
| 首版用户模型 | 单用户；“登录”特指 Bilibili Cookie 登录态，不建设独立账号体系 |

## 1. 文档目的

本文档用于定义一个 Bilibili 视频解析、内容分析与下载工具的产品范围、用户流程、功能要求、技术边界、验收标准和迭代计划，可作为产品设计、前后端开发、测试和后续排期的共同依据。

本文档中的 Python、Vue 3、Element Plus、ECharts 等均为拟定方案，当前仓库尚无现成代码实现。
...
4. 用户可选择视频、音频、封装和兼容策略，创建可追踪的下载任务。
5. 系统可按需完成媒体参数、字幕/ASR、OCR、镜头和内容摘要分析。
6. 所有长任务具有清晰进度、失败原因和可恢复能力。
7. Cookie、临时播放地址和账号信息得到严格保护。
8. 核心功能在手机、平板和桌面浏览器上均可使用。
...
- 默认公开分享或长期托管下载产物。
- 首版同时覆盖番剧、课程、直播、互动视频、充电专属、合集批量下载等所有内容形态。
- 训练自有大模型或承诺 AI 分析完全准确。
- 建设独立的应用用户注册、登录、找回密码体系。
...
3. 作为已保存 Cookie 的用户，我仍希望明确控制本次请求是否使用登录态，避免仅因保存 Cookie 就自动携带。
4. 作为下载用户，我希望看到分辨率、帧率、编码、码率、音频和预估大小，再选择下载方案。
5. 作为普通用户，我希望系统提供“最佳画质”和“最佳兼容”预设，而不必理解所有编码参数。
6. 作为分析用户，我希望单独选择媒体分析、ASR、OCR、关键帧或内容摘要，避免不必要的耗时。
7. 作为移动端用户，我希望通过卡片和底部抽屉完成解析、选择和任务查看。
8. 作为重视隐私的用户，我希望 Cookie 不被前端回显、日志记录或发送给无关域名，并可一键彻底删除。
...
关键原则：解析结果中的媒体 URL 是短期资源。系统保存流规格，不把临时 URL 当作永久地址；任务开始前必须重新获取并验证。
...
| 页面 | 主要内容 |
| --- | --- |
| 首页/解析页 | 链接输入、最近解析、匿名/登录状态、支持范围说明 |
| 视频详情页 | 基础信息、分 P、清晰度、媒体分析、内容分析、下载操作 |
| 任务中心 | 下载/合并/转码/分析任务、进度、速度、错误、重试与取消 |
| 产物/历史页 | 已下载文件、字幕、转写、关键帧、报告、保存与删除 |
| 设置页 | Cookie 登录态、下载、存储、并发、分析模型和隐私设置 |
...
3. 媒体流：视频清晰度、编码、帧率、HDR、码率、预估大小和音频选择。
...
- 输入框接受完整 URL、BV 号和 AV 号；MVP 对短链接可提示后续支持或安全跟随跳转。
- 自动移除 `spm_id_from`、`vd_source` 等不影响视频定位的跟踪参数。
- 保留合法的 `p` 等业务参数，并在多 P 视频中默认选中对应分 P。
...
- `codec`、`container`、`width`、`height`、`fps`、`bitrate`。
...
| 错误场景 | 用户提示 | 推荐操作 |
| --- | --- | --- |
| 链接格式不支持 | 无法识别该链接，请使用普通 BV/AV 视频链接 | 返回首页修改 |
...
4. **首版内容类型**：普通投稿视频与分 P；其他类型后续逐项支持。
