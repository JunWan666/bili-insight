...
- 1440 桌面内容区至少容纳四列最近解析卡片，超宽屏可继续增加列数；卡片宽度不足时自动降列。
...
- 设置分组与 URL query 同步，刷新、前进后退和外部深链均恢复正确分组。
- 桌面设置内容区不再重复显示独立分组菜单；侧栏收起、平板和手机布局通过紧凑分组选择器保持全部功能可达。

### 24.4 v0.5 验收门禁

- 1440×900 最近解析网格至少四列，卡片高度与信息字段有自动化断言。
- 桌面设置二级菜单展开、收起、激活状态和 query 同步具备 E2E 证据。
