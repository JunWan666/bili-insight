...
| 一键部署脚本 | `deploy.sh` 通过 Bash 语法与内置自检，Linux 默认 `/opt/bili-insight` 并启用 Readline 输入；ASCII `install.ps1` 通过 `irm | iex` 启动带 BOM 的完整中文管理器；`deploy.ps1` 通过 Windows PowerShell 5 解析、内置自检和真实 Docker 状态/健康查询；两端覆盖现有网络配置保留、显式参数优先与危险目录保护 |
...
- 完整 Playwright 七项目矩阵、最近解析五列竖向卡门禁、七个独立设置路径、同风格侧栏二级菜单和手机单屏复验均已完成。
- 当前无阻断交付项；剩余边界见下节。
