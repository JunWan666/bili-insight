  ...
  <div class="video-detail">
    ...
    <template v-else-if="video">
      ...
      <section v-if="video.parts.length" class="part-bar surface-card">
        <div><span>当前分 P</span><strong>{{ selectedPart?.pageNumber }} / {{ video.parts.length }}</strong></div>
        <div class="part-controls">
          <el-select :modelValue="selectedPartId" filterable placeholder="选择分 P" @update:modelValue="changePart">
            <el-option v-for="part in video.parts" :key="part.id" :value="part.id" :label="`P${part.pageNumber} · ${part.title}`"><span class="part-option">P{{ part.pageNumber }} · {{ part.title }}<small>{{ formatDuration(part.duration) }}</small></span></el-option>
          </el-select>
...
<style scoped>
.video-detail { max-width: 1280px; margin: 0 auto; }
...
.video-head { display: grid; grid-template-columns: minmax(300px, 420px) 1fr; gap: 28px; padding: 24px; }
...
.video-copy h1 { margin: 13px 0 14px; font-size: clamp(23px, 3vw, 34px); line-height: 1.32; letter-spacing: -.035em; overflow-wrap: anywhere; }
...
.description { display: -webkit-box; margin: 14px 0 0; overflow: hidden; color: var(--text-secondary); line-height: 1.7; white-space: pre-wrap; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }
...
.access-banner { display: flex; align-items: center; gap: 14px; margin: 16px 0; padding: 16px 18px; border: 1px solid var(--line); border-radius: 15px; background: var(--surface); }
...
.part-bar { display: flex; align-items: center; gap: 16px; margin-bottom: 16px; padding: 12px 16px; }
...
.part-controls { display: flex; flex: 1; align-items: center; gap: 10px; min-width: 0; }.part-controls .el-select { flex: 1; max-width: 580px; }
...
.workspace-body { min-height: 400px; padding: 24px; }
